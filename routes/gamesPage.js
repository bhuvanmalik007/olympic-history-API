...
const topWinnerQuery = (year, season) => `select distinct a.Ath_Name, y.cnt,team_name, spt_name  from athlete a,
(select * from
(select id, count(*) cnt from participates_in p, event e, game g
where e.event_id=p.event_id and e.game_name=g.game_name and g.game_name='${year} ${season}' and medal<>'na' group by id order by count(*) desc)  where  rownum<11)y,
participates_in p1,event e1 where a.id=y.id and p1.id=a.id and p1.event_id=e1.event_id order by y.cnt desc`
